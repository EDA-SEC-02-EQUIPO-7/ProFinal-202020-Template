...
def newIndexHeapmax(cmpfunction):
    """
    Crea un cola de prioridad indexada orientada a mayor
...
def delMax(iheap):
    ...
    try:
        if (iheap['size'] > 0):
            ...
            lt.deleteElement(iheap["elements"],iheap['size'])
            iheap['size'] -= 1
            sink(iheap, 1)
...
def swim(iheap, pos):
    ...
    try:
        while (pos > 1):
            ...
            if not(greater(iheap, parent, element)):
...
def sink(iheap, pos):
    ...
    try:
        ...
        while ((2*pos <= size)):
            j = 2*pos
            if (j < size):
                if (not greater(iheap, lt.getElement(iheap['elements'], j),
                           lt.getElement(iheap['elements'], (j+1)))):
                    j += 1
            if (greater(iheap, lt.getElement(iheap['elements'], pos),
